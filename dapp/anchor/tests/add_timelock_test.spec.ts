...
describe('add_timelock instruction test', () => {
  ...
  it('creates a timelockAccount and adds a timelock module to the escrow', async () => {
    ...
    const timelockModule = escrowAccount.modules.find(mod => {
      return mod.moduleType.timelock !== undefined;
    });
    expect(timelockModule).toBeTruthy();
    expect(timelockModule?.key.toBase58()).toBe(timelockKey.toBase58());
  })

  it('fails to add a second timelock module', async () => {
    await program.methods.addTimelock(
        Array.from(escrowId),
        new anchor.BN(nowPlusOneDay)
    )
